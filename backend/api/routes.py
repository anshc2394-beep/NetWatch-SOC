...
Contains all Flask routes for the NetWatch-SOC application.
"""
...
from backend.models.models import init_db, db, User
...
def create_app():
    ...
    def api_anomaly_detail(anomaly_id):
        ...
        if session.get('demo_mode'):
            # Find anomaly in simulation cache
            anomaly = next((f for f in SIMULATION_CACHE["flows"] if str(f.get('timestamp', 0)) == anomaly_id), None)
            if not anomaly:
            ...
            return jsonify({
                "anomaly": anomaly,
                "time_window": {
                    "start": anomaly['timestamp'] - 5,
                    "end": anomaly['timestamp']
                },
                "top_ips": [{"ip": anomaly['src_ip'], "packets": anomaly['pkt_count']}],
                "protocol_breakdown": {"TCP": 100},
                "packet_vs_byte_graph": {
                    "packets": [anomaly['pkt_count']],
                    "bytes": [anomaly['byte_count']]
                }
            })
        
        # Find anomaly in alerts_store
        with detector._results_lock:
            anomaly = next((a for a in detector.alerts_store if str(a.get('timestamp', 0)) == anomaly_id), None)
        
        if not anomaly:
            return jsonify({"error": "Anomaly not found"}), 404
        
        # Get related flows in time window
        window_start = anomaly.get('timestamp', 0) - 5
        window_end = anomaly.get('timestamp', 0)
        
        with detector._results_lock:
            related_flows = [f for f in detector.results_store 
        ...
        for flow in related_flows:
            ...
            ip_counts[src_ip] = ip_counts.get(src_ip, 0) + flow.get('pkt_count', 0)
            
            # Mock protocol (assuming TCP for now)
            protocol_counts['TCP'] = protocol_counts.get('TCP', 0) + flow.get('pkt_count', 0)
            
            packet_data.append(flow.get('pkt_count', 0))
            byte_data.append(flow.get('byte_count', 0))
